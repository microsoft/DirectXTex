    ...
    uint32_t                                                      tflags) noexcept
{
    ...
    if (tflags & TEXP_SCANLINE_SETALPHA)
    {
        ...
        case DXGI_FORMAT_AYUV:
            if (inSize >= 4 && outSize >= 4)
            {
                const uint32_t alpha
                    = (format == DXGI_FORMAT_R8G8B8A8_SNORM || format == DXGI_FORMAT_R8G8B8A8_SINT) ? 0x7f000000 : 0xff000000;
        ...
        case WIN11_DXGI_FORMAT_A4B4G4R4_UNORM:
            ...
            //-----------------------------------------------------------------------------
        case DXGI_FORMAT_A8_UNORM: memset(pDestination, 0xff, outSize); return;
...
DirectX::Internal::LoadScanline(XMVECTOR* pDestination, size_t count, const void* pSource, size_t size, DXGI_FORMAT format) noexcept
{
    ...
    case DXGI_FORMAT_R9G9B9E5_SHAREDEXP: LOAD_SCANLINE3(XMFLOAT3SE, XMLoadFloat3SE, g_XMIdentityR3)
    ...
    case XBOX_DXGI_FORMAT_R10G10B10_7E3_A2_FLOAT:
        ...
        if (size >= sizeof(XMUDECN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUDECN4) + 1); icount += sizeof(XMUDECN4))
            {
                ...
                XMVECTORF32 vResult = {
                    { { FloatFrom7e3(sPtr->x), FloatFrom7e3(sPtr->y), FloatFrom7e3(sPtr->z), static_cast<float>(sPtr->v >> 30) / 3.0f } }
                };
    ...
    case XBOX_DXGI_FORMAT_R10G10B10_6E4_A2_FLOAT:
        ...
        if (size >= sizeof(XMUDECN4))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(XMUDECN4) + 1); icount += sizeof(XMUDECN4))
            {
                ...
                XMVECTORF32 vResult = {
                    { { FloatFrom6e4(sPtr->x), FloatFrom6e4(sPtr->y), FloatFrom6e4(sPtr->z), static_cast<float>(sPtr->v >> 30) / 3.0f } }
                };
...
_Use_decl_annotations_ bool DirectX::Internal::StoreScanline(void* pDestination,
    size_t                                                         size,
    DXGI_FORMAT                                                    format,
    ...
    float                                                          threshold) noexcept
{
    ...
    *reinterpret_cast<uint8_t*>(pDestination) = 0;
    ...
    case DXGI_FORMAT_D24_UNORM_S8_UINT:
        if (size >= sizeof(uint32_t))
        {
            ...
            for (size_t icount = 0; icount < (size - sizeof(uint32_t) + 1); icount += sizeof(uint32_t))
            {
                ...
                *dPtr++ = (static_cast<uint32_t>(f.x * 16777215.f) & 0xFFFFFF) | ((static_cast<uint32_t>(f.y) & 0xFF) << 24);
            }
            return true;
        }
        return false;
    ...
    case DXGI_FORMAT_R9G9B9E5_SHAREDEXP: STORE_SCANLINE(XMFLOAT3SE, StoreFloat3SE)
...
_Use_decl_annotations_ HRESULT DirectX::Internal::ConvertToR16G16B16A16(const Image& srcImage, ScratchImage& image) noexcept
{
    ...
    for (size_t h = 0; h < srcImage.height; ++h)
    {
        ...
        XMConvertFloatToHalfStream(reinterpret_cast<HALF*>(pDest),
            sizeof(HALF),
            reinterpret_cast<float*>(scanline.get()),
            sizeof(float),
            srcImage.width * 4);
...
_Use_decl_annotations_ HRESULT DirectX::Internal::ConvertFromR16G16B16A16(const Image& srcImage, const Image& destImage) noexcept
{
    ...
    for (size_t h = 0; h < srcImage.height; ++h)
    {
        XMConvertHalfToFloatStream(reinterpret_cast<float*>(scanline.get()),
            sizeof(float),
            reinterpret_cast<const HALF*>(pSrc),
            sizeof(HALF),
            srcImage.width * 4);
    ...
    float                                                                threshold) noexcept
{
    ...
    case DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
    case DXGI_FORMAT_B8G8R8A8_UNORM_SRGB:
    case DXGI_FORMAT_B8G8R8X8_UNORM_SRGB:  flags |= TEX_FILTER_SRGB; break;
    ...
    case DXGI_FORMAT_B8G8R8X8_UNORM:
    case DXGI_FORMAT_B4G4R4A4_UNORM:
    case WIN11_DXGI_FORMAT_A4B4G4R4_UNORM: break;
...
#pragma prefast(suppress : 25004, "Signature must match bsearch");
    int __cdecl ConvertCompare(const void* ptr1, const void* ptr2) noexcept
    {
        ...
        if (p1->format == p2->format)
            ...
            return (p1->format < p2->format) ? -1 : 1;
    }
} // namespace
    ...
    TEX_FILTER_FLAGS                                                     flags) noexcept
{
    ...
    case DXGI_FORMAT_B8G8R8A8_UNORM_SRGB:
    case DXGI_FORMAT_B8G8R8X8_UNORM_SRGB:
    case DXGI_FORMAT_BC7_UNORM_SRGB:             flags |= TEX_FILTER_SRGB_IN; break;

    case DXGI_FORMAT_A8_UNORM:
    case DXGI_FORMAT_R10G10B10_XR_BIAS_A2_UNORM: flags &= ~TEX_FILTER_SRGB_IN; break;
    ...
    case DXGI_FORMAT_B8G8R8A8_UNORM_SRGB:
    case DXGI_FORMAT_B8G8R8X8_UNORM_SRGB:
    case DXGI_FORMAT_BC7_UNORM_SRGB:             flags |= TEX_FILTER_SRGB_OUT; break;

    case DXGI_FORMAT_A8_UNORM:
    case DXGI_FORMAT_R10G10B10_XR_BIAS_A2_UNORM: flags &= ~TEX_FILTER_SRGB_OUT; break;
    ...
    if (diffFlags != 0)
    {
        if (diffFlags & CONVF_DEPTH)
        {
            ...
            else
            {
                ...
                switch (
                    flags & (TEX_FILTER_RGB_COPY_RED | TEX_FILTER_RGB_COPY_GREEN | TEX_FILTER_RGB_COPY_BLUE | TEX_FILTER_RGB_COPY_ALPHA))
                {
            ...
            else if ((out->flags & CONVF_RGB_MASK) == (CONVF_R | CONVF_G))
            {
                if ((flags & TEX_FILTER_RGB_COPY_ALPHA) && (in->flags & CONVF_A))
                {
                    ...
                    switch (static_cast<int>(
                        flags
                        & (TEX_FILTER_RGB_COPY_RED | TEX_FILTER_RGB_COPY_GREEN | TEX_FILTER_RGB_COPY_BLUE | TEX_FILTER_RGB_COPY_ALPHA)))
                    {
    ...
    {                                                                                                                                   \
        ...
        {                                                                                                                               \
            ...
            if (norm && clampzero)                                                                                                      \
                ...
                v = XMVectorClamp(v, g_XMZero, scalev);                                                                                 \
            else if (norm)                                                                                                              \
                v = XMVectorClamp(v, g_XMNegativeOne, g_XMOne);                                                                         \
            else                                                                                                                        \
                v = XMVectorClamp(v, XMVectorAdd(XMVectorNegate(scalev), g_XMOne), scalev);                                             \
            v = XMVectorAdd(v, vError);                                                                                                 \
            if (norm)                                                                                                                   \
                v = XMVectorMultiply(v, scalev);                                                                                        \
                                                                                                                                        \
            XMVECTOR target;                                                                                                            \
            if (pDiffusionErrors)                                                                                                       \
            {                                                                                                                           \
                ...
                if (norm)                                                                                                               \
                    vError = XMVectorDivide(vError, scalev);                                                                            \
                                                                                                                                        \
    ...
    XMVECTOR*                                                            pDiffusionErrors) noexcept
{
    ...
    *reinterpret_cast<uint8_t*>(pDestination) = 0;
    ...
    switch (static_cast<int>(format))
    {
    case DXGI_FORMAT_R16G16B16A16_UNORM: STORE_SCANLINE(XMUSHORTN4, g_Scale16pc, true, true, uint16_t, 0xFFFF, y, false)

    case DXGI_FORMAT_R16G16B16A16_UINT:  STORE_SCANLINE(XMUSHORT4, g_Scale16pc, true, false, uint16_t, 0xFFFF, y, false)

    case DXGI_FORMAT_R16G16B16A16_SNORM: STORE_SCANLINE(XMSHORTN4, g_Scale15pc, false, true, int16_t, 0xFFFF, y, false)

    case DXGI_FORMAT_R16G16B16A16_SINT:  STORE_SCANLINE(XMSHORT4, g_Scale15pc, false, false, int16_t, 0xFFFF, y, false)

    case DXGI_FORMAT_R10G10B10A2_UNORM:  STORE_SCANLINE(XMUDECN4, g_Scale10pc, true, true, uint16_t, 0x3FF, y, false)

    case DXGI_FORMAT_R10G10B10A2_UINT:   STORE_SCANLINE(XMUDEC4, g_Scale10pc, true, false, uint16_t, 0x3FF, y, false)
    ...
    case DXGI_FORMAT_R8G8B8A8_UNORM:
    case DXGI_FORMAT_R8G8B8A8_UNORM_SRGB: STORE_SCANLINE(XMUBYTEN4, g_Scale8pc, true, true, uint8_t, 0xFF, y, false)

    case DXGI_FORMAT_R8G8B8A8_UINT:       STORE_SCANLINE(XMUBYTE4, g_Scale8pc, true, false, uint8_t, 0xFF, y, false)

    case DXGI_FORMAT_R8G8B8A8_SNORM:      STORE_SCANLINE(XMBYTEN4, g_Scale7pc, false, true, int8_t, 0xFF, y, false)

    case DXGI_FORMAT_R8G8B8A8_SINT:       STORE_SCANLINE(XMBYTE4, g_Scale7pc, false, false, int8_t, 0xFF, y, false)

    case DXGI_FORMAT_R16G16_UNORM:        STORE_SCANLINE2(XMUSHORTN2, g_Scale16pc, true, true, uint16_t, 0xFFFF, y)

    case DXGI_FORMAT_R16G16_UINT:         STORE_SCANLINE2(XMUSHORT2, g_Scale16pc, true, false, uint16_t, 0xFFFF, y)

    case DXGI_FORMAT_R16G16_SNORM:        STORE_SCANLINE2(XMSHORTN2, g_Scale15pc, false, true, int16_t, 0xFFFF, y)

    case DXGI_FORMAT_R16G16_SINT:         STORE_SCANLINE2(XMSHORT2, g_Scale15pc, false, false, int16_t, 0xFFFF, y)
    ...
    case DXGI_FORMAT_D24_UNORM_S8_UINT:
        if (size >= sizeof(uint32_t))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                if (dPtr >= ePtr)
                    break;
                *dPtr = (static_cast<uint32_t>(tmp.x) & 0xFFFFFF) | ((static_cast<uint32_t>(tmp.y) & 0xFF) << 24);
            }
            return true;
        }
        return false;

    case DXGI_FORMAT_R8G8_UNORM: STORE_SCANLINE2(XMUBYTEN2, g_Scale8pc, true, true, uint8_t, 0xFF, y)

    case DXGI_FORMAT_R8G8_UINT:  STORE_SCANLINE2(XMUBYTE2, g_Scale8pc, true, false, uint8_t, 0xFF, y)

    case DXGI_FORMAT_R8G8_SNORM: STORE_SCANLINE2(XMBYTEN2, g_Scale7pc, false, true, int8_t, 0xFF, y)

    case DXGI_FORMAT_R8G8_SINT:  STORE_SCANLINE2(XMBYTE2, g_Scale7pc, false, false, int8_t, 0xFF, y)

    case DXGI_FORMAT_D16_UNORM:
    case DXGI_FORMAT_R16_UNORM:  STORE_SCANLINE1(uint16_t, g_Scale16pc, true, true, 0xFFFF, y, false)

    case DXGI_FORMAT_R16_UINT:   STORE_SCANLINE1(uint16_t, g_Scale16pc, true, false, 0xFFFF, y, false)

    case DXGI_FORMAT_R16_SNORM:  STORE_SCANLINE1(int16_t, g_Scale15pc, false, true, 0xFFFF, y, false)

    case DXGI_FORMAT_R16_SINT:   STORE_SCANLINE1(int16_t, g_Scale15pc, false, false, 0xFFFF, y, false)

    case DXGI_FORMAT_R8_UNORM:   STORE_SCANLINE1(uint8_t, g_Scale8pc, true, true, 0xFF, y, false)

    case DXGI_FORMAT_R8_UINT:    STORE_SCANLINE1(uint8_t, g_Scale8pc, true, false, 0xFF, y, false)

    case DXGI_FORMAT_R8_SNORM:   STORE_SCANLINE1(int8_t, g_Scale7pc, false, true, 0xFF, y, false)

    case DXGI_FORMAT_R8_SINT:    STORE_SCANLINE1(int8_t, g_Scale7pc, false, false, 0xFF, y, false)

    case DXGI_FORMAT_A8_UNORM:   STORE_SCANLINE1(uint8_t, g_Scale8pc, true, true, 0xFF, y, true)
    ...
    case DXGI_FORMAT_B8G8R8A8_UNORM:
    case DXGI_FORMAT_B8G8R8A8_UNORM_SRGB: STORE_SCANLINE(XMUBYTEN4, g_Scale8pc, true, true, uint8_t, 0xFF, y, true)
    ...
    case DXGI_FORMAT_B4G4R4A4_UNORM: STORE_SCANLINE(XMUNIBBLE4, g_Scale4pc, true, true, uint8_t, 0xF, y, true)
    ...
    case XBOX_DXGI_FORMAT_R10G10B10_SNORM_A2_UNORM: STORE_SCANLINE(XMXDECN4, g_Scale9pc, false, true, uint16_t, 0x3FF, y, false)
    ...
    default: return StoreScanline(pDestination, size, format, pSource, count, threshold);
    }
}
...
namespace
{
        ...
        _Out_ WICPixelFormatGUID&                      targetGUID) noexcept
    {
        ...
        if (sformat == DXGI_FORMAT_R16G16B16A16_FLOAT || sformat == DXGI_FORMAT_R16_FLOAT || tformat == DXGI_FORMAT_R16G16B16A16_FLOAT
    ...
    //-------------------------------------------------------------------------------------
    HRESULT ConvertUsingWIC(_In_ const Image& srcImage,
...
DirectX::Convert(const Image& srcImage, DXGI_FORMAT format, TEX_FILTER_FLAGS filter, float threshold, ScratchImage& image) noexcept
{
...
_Use_decl_annotations_ HRESULT DirectX::ConvertEx(const Image& srcImage,
    DXGI_FORMAT                                                format,
    const ConvertOptions&                                      options,
    ScratchImage&                                              image,
    std::function<bool __cdecl(size_t, size_t)>                statusCallback)
{
    if ((srcImage.format == format) || !IsValid(format) || !IsValid(srcImage.format))
        return E_INVALIDARG;

    if (!srcImage.pixels)
        return E_POINTER;

    if (IsCompressed(srcImage.format) || IsCompressed(format) || IsPlanar(srcImage.format) || IsPlanar(format)
        || IsPalettized(srcImage.format) || IsPalettized(format) || IsTypeless(srcImage.format) || IsTypeless(format))
...
_Use_decl_annotations_ HRESULT DirectX::ConvertEx(const Image* srcImages,
    size_t                                                     nimages,
    const TexMetadata&                                         metadata,
    DXGI_FORMAT                                                format,
    const ConvertOptions&                                      options,
    ...
    std::function<bool __cdecl(size_t, size_t)>                statusCallback)
{
    if (!srcImages || !nimages || (metadata.format == format) || !IsValid(format) || !IsValid(metadata.format))
        return E_INVALIDARG;

    if (IsCompressed(metadata.format) || IsCompressed(format) || IsPlanar(metadata.format) || IsPlanar(format)
        || IsPalettized(metadata.format) || IsPalettized(format) || IsTypeless(metadata.format) || IsTypeless(format))
        return HRESULT_E_NOT_SUPPORTED;

    if ((metadata.width > UINT32_MAX) || (metadata.height > UINT32_MAX))
        return E_INVALIDARG;

    if (statusCallback && nimages == 1 && !metadata.IsVolumemap() && metadata.mipLevels == 1 && metadata.arraySize == 1)
    {
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::ConvertToSinglePlane(const Image& srcImage, ScratchImage& image) noexcept
{
...
DirectX::FORMAT_TYPE DirectX::FormatDataType(_In_ DXGI_FORMAT fmt) noexcept
{
    ...
    {
    case CONVF_FLOAT: return FORMAT_TYPE_FLOAT;

    case CONVF_UNORM: return FORMAT_TYPE_UNORM;

    case CONVF_UINT:  return FORMAT_TYPE_UINT;

    case CONVF_SNORM: return FORMAT_TYPE_SNORM;

    case CONVF_SINT:  return FORMAT_TYPE_SINT;

    default:          return FORMAT_TYPE_TYPELESS;
    }
}
