...
#define TRILINEAR_INTERPOLATE(res, x, y, z, r0, r1, r2, r3)                                                                      \
    {                                                                                                                            \
        const XMVECTOR a0                                                                                                        \
            = XMVectorScale(XMVectorAdd(XMVectorScale((r0)[x.u0], x.weight0), XMVectorScale((r0)[x.u1], x.weight1)), y.weight0); \
        const XMVECTOR a1                                                                                                        \
            = XMVectorScale(XMVectorAdd(XMVectorScale((r1)[x.u0], x.weight0), XMVectorScale((r1)[x.u1], x.weight1)), y.weight1); \
        const XMVECTOR a2                                                                                                        \
            = XMVectorScale(XMVectorAdd(XMVectorScale((r2)[x.u0], x.weight0), XMVectorScale((r2)[x.u1], x.weight1)), y.weight0); \
        const XMVECTOR a3                                                                                                        \
            = XMVectorScale(XMVectorAdd(XMVectorScale((r3)[x.u0], x.weight0), XMVectorScale((r3)[x.u1], x.weight1)), y.weight1); \
        ...
        inline void CreateCubicFilter(_In_ size_t source,
            _In_ size_t                           dest,
            _In_ bool                             wrap,
            _In_ bool                             mirror,
            _Out_writes_(dest) CubicFilter*       cf) noexcept
        {
            assert(source > 0);
            assert(dest > 0);
            assert(cf != nullptr);
...
#define CUBIC_INTERPOLATE(res, dx, p0, p1, p2, p3)                                                                             \
    {                                                                                                                          \
        ...
        res                 = XMVectorAdd(XMVectorAdd(XMVectorAdd(a0, XMVectorMultiply(a1, vdx)), XMVectorMultiply(a2, vdx2)), \
            XMVectorMultiply(a3, vdx3));                                                                       \
    }
        ...
        inline HRESULT
        CreateTriangleFilter(_In_ size_t source, _In_ size_t dest, _In_ bool wrap, _Inout_ std::unique_ptr<Filter>& tf) noexcept
        {
            assert(source > 0);
            assert(dest > 0);
