...
#pragma pack(pop)
    ...
    // Decodes TGA header
    //-------------------------------------------------------------------------------------
    HRESULT DecodeTGAHeader(_In_reads_bytes_(size) const uint8_t* pSource,
        size_t                                                    size,
        TGA_FLAGS                                                 flags,
        ...
        _Inout_opt_ uint32_t*                                     convFlags) noexcept
    {
        ...
        {
        case TGA_NO_IMAGE:
        case TGA_COLOR_MAPPED_RLE: return HRESULT_E_NOT_SUPPORTED;

        case TGA_COLOR_MAPPED:
            if (pHeader->bColorMapType != 1 || pHeader->wColorMapLength == 0 || pHeader->bBitsPerPixel != 8)
            {
                return HRESULT_E_NOT_SUPPORTED;
            }
        ...
        case TGA_TRUECOLOR_RLE:
            if (pHeader->bColorMapType != 0 || pHeader->wColorMapLength != 0)
            {
                return HRESULT_E_NOT_SUPPORTED;
            }
            ...
            {
            case 16: metadata.format = DXGI_FORMAT_B5G5R5A1_UNORM; break;
            ...
            case 32: metadata.format = (flags & TGA_FLAGS_BGR) ? DXGI_FORMAT_B8G8R8A8_UNORM : DXGI_FORMAT_R8G8B8A8_UNORM; break;
        ...
        case TGA_BLACK_AND_WHITE_RLE:
            ...
            switch (pHeader->bBitsPerPixel)
            {
            case 8:  metadata.format = DXGI_FORMAT_R8_UNORM; break;
    ...
    //-------------------------------------------------------------------------------------
    HRESULT ReadPalette(_In_reads_(TGA_HEADER_LEN) const uint8_t* header,
        ...
        size_t&                                                   colorMapSize) noexcept
    {
        ...
        if (pHeader->bColorMapType != 1 || pHeader->wColorMapLength == 0 || pHeader->wColorMapLength > 256 || pHeader->bColorMapSize != 24)
        {
            return HRESULT_E_NOT_SUPPORTED;
        }
    ...
    //-------------------------------------------------------------------------------------
    HRESULT UncompressPixels(_In_reads_bytes_(size) const void* pSource,
        size_t                                                  size,
        TGA_FLAGS                                               flags,
        ...
        _In_ uint32_t                                           convFlags) noexcept
    {
        ...
        HRESULT hr = ComputePitch(image->format,
            image->width,
            image->height,
            rowPitch,
            slicePitch,
        ...
        case DXGI_FORMAT_R8_UNORM:
            for (size_t y = 0; y < image->height; ++y)
            {
                ...
                uint8_t* dPtr
                    = image->pixels + (image->rowPitch * ((convFlags & CONV_FLAGS_INVERTY) ? y : (image->height - y - 1))) + offset;
        ...
        case DXGI_FORMAT_B5G5R5A1_UNORM: {
            ...
            for (size_t y = 0; y < image->height; ++y)
            {
                ...
                auto dPtr = reinterpret_cast<uint16_t*>(
                                image->pixels + (image->rowPitch * ((convFlags & CONV_FLAGS_INVERTY) ? y : (image->height - y - 1))))
                            + offset;
        ...
        case DXGI_FORMAT_R8G8B8A8_UNORM: {
            ...
            for (size_t y = 0; y < image->height; ++y)
            {
                ...
                auto dPtr = reinterpret_cast<uint32_t*>(
                                image->pixels + (image->rowPitch * ((convFlags & CONV_FLAGS_INVERTY) ? y : (image->height - y - 1))))
                            + offset;
    ...
    //-------------------------------------------------------------------------------------
    HRESULT CopyPixels(_In_reads_bytes_(size) const void* pSource,
        size_t                                            size,
        TGA_FLAGS                                         flags,
        _In_ const Image*                                 image,
...
#pragma warning(suppress : 6001 6101) // In the case where outSize is insufficient we do not write to pDestination
    void Copy24bppScanline(_Out_writes_bytes_(outSize) void* pDestination,
        _In_ size_t                                          outSize,
    ...
    void SetExtension(_In_ TGA_EXTENSION* ext, TGA_FLAGS flags, const TexMetadata& metadata) noexcept
    {
        ...
        {
        default:
        case TEX_ALPHA_MODE_UNKNOWN:       ext->bAttributesType = HasAlpha(metadata.format) ? TGA_ATTRIBUTE_UNDEFINED : TGA_ATTRIBUTE_NONE; break;

        case TEX_ALPHA_MODE_STRAIGHT:      ext->bAttributesType = TGA_ATTRIBUTE_ALPHA; break;

        case TEX_ALPHA_MODE_PREMULTIPLIED: ext->bAttributesType = TGA_ATTRIBUTE_PREMULTIPLIED; break;

        case TEX_ALPHA_MODE_OPAQUE:        ext->bAttributesType = TGA_ATTRIBUTE_IGNORED; break;

        case TEX_ALPHA_MODE_CUSTOM:        ext->bAttributesType = TGA_ATTRIBUTE_UNDEFINED; break;
        }
    ...
    DXGI_FORMAT
    GetSRGBFromExtension(_In_opt_ const TGA_EXTENSION* ext, DXGI_FORMAT format, TGA_FLAGS flags, _In_opt_ ScratchImage* image) noexcept
    {
        bool sRGB = false;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::GetMetadataFromTGAMemory(const uint8_t* pSource,
    size_t                                                                      size,
    TGA_FLAGS                                                                   flags,
    TexMetadata&                                                                metadata) noexcept
{
    ...
    if (size >= sizeof(TGA_FOOTER))
    {
        ...
        {
            if (footer->dwExtensionOffset != 0 && ((footer->dwExtensionOffset + sizeof(TGA_EXTENSION)) <= size))
            {
...
_Use_decl_annotations_ HRESULT DirectX::GetMetadataFromTGAFile(const wchar_t* szFile, TGA_FLAGS flags, TexMetadata& metadata) noexcept
{
    ...
    TGA_EXTENSION        extData = {};
    {
        ...
        if (memcmp(footer.Signature, g_Signature, sizeof(g_Signature)) == 0)
        {
            if (footer.dwExtensionOffset != 0 && ((footer.dwExtensionOffset + sizeof(TGA_EXTENSION)) <= len))
            {
#ifdef _WIN32
                ...
                {
                    if (ReadFile(hFile.get(), &extData, sizeof(TGA_EXTENSION), &bytesRead, nullptr) && bytesRead == sizeof(TGA_EXTENSION))
                    {
                        ext = &extData;
                        metadata.SetAlphaMode(GetAlphaModeFromExtension(ext));
                    }
...
DirectX::LoadFromTGAMemory(const uint8_t* pSource, size_t size, TGA_FLAGS flags, TexMetadata* metadata, ScratchImage& image) noexcept
{
    if (!pSource || size == 0)
        return E_INVALIDARG;
    ...
    if (convFlags & CONV_FLAGS_PALETTED)
    {
        ...
        hr = ReadPalette(static_cast<const uint8_t*>(pSource), pColorMap, remaining, flags, palette, paletteOffset);
        if (FAILED(hr))
            return hr;
    }
    ...
    ScratchImage&                                                      image) noexcept
{
    ...
    else // RLE || EXPAND || INVERTX || PALETTED || !INVERTY
    {
        ...
        if (convFlags & CONV_FLAGS_RLE)
        {
            hr = UncompressPixels(temp.get() + paletteOffset, remaining - paletteOffset, flags, image.GetImage(0, 0, 0), convFlags);
        }
        else
        {
            hr = CopyPixels(temp.get() + paletteOffset, remaining - paletteOffset, flags, image.GetImage(0, 0, 0), convFlags, palette);
        }
    ...
    const TexMetadata*                                               metadata) noexcept
{
    ...
    hr = blob.Initialize(TGA_HEADER_LEN + slicePitch + (metadata ? sizeof(TGA_EXTENSION) : 0) + sizeof(TGA_FOOTER));
    if (FAILED(hr))
        return hr;
...
namespace DirectX
{
    HRESULT __cdecl GetMetadataFromTGAFile(_In_z_ const __wchar_t* szFile, _In_ TGA_FLAGS flags, _Out_ TexMetadata& metadata) noexcept
    {
        return GetMetadataFromTGAFile(reinterpret_cast<const unsigned short*>(szFile), flags, metadata);
    }

    HRESULT __cdecl LoadFromTGAFile(_In_z_ const __wchar_t* szFile,
