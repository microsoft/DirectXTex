...
#pragma prefast(suppress : 25000, "FXMVECTOR is 16 bytes")
    ...
    void
    EvaluateRow(_In_reads_(width) const XMVECTOR* pSource, _Out_writes_(width + 2) float* pDest, size_t width, CNMAP_FLAGS flags) noexcept
    {
    ...
    HRESULT ComputeNMap(_In_ const Image& srcImage,
        _In_ CNMAP_FLAGS                  flags,
        _In_ float                        amplitude,
        _In_ DXGI_FORMAT                  format,
        _In_ const Image&                 normalMap) noexcept
    {
        ...
        for (size_t y = 0; y < height; ++y)
        {
            ...
            for (size_t x = 0; x < width; ++x)
            {
                ...
                if (convFlags & CONVF_UNORM)
                {
                    ...
                    const XMVECTOR n1
                        = XMVectorMultiplyAdd((flags & CNMAP_INVERT_SIGN) ? g_XMNegativeOneHalf : g_XMOneHalf, normal, g_XMOneHalf);
...
DirectX::ComputeNormalMap(const Image& srcImage, CNMAP_FLAGS flags, float amplitude, DXGI_FORMAT format, ScratchImage& normalMap) noexcept
{
    if (!srcImage.pixels)
        return E_POINTER;
    ...
    case CNMAP_CHANNEL_LUMINANCE: break;

    default:                      return E_INVALIDARG;
    }

    if (IsCompressed(format) || IsCompressed(srcImage.format) || IsTypeless(format) || IsTypeless(srcImage.format) || IsPlanar(format)
        || IsPlanar(srcImage.format) || IsPalettized(format) || IsPalettized(srcImage.format))
        return HRESULT_E_NOT_SUPPORTED;
    ...
    ScratchImage&                                                     normalMaps) noexcept
{
    ...
    if (IsCompressed(format) || IsCompressed(metadata.format) || IsTypeless(format) || IsTypeless(metadata.format) || IsPlanar(format)
        || IsPlanar(metadata.format) || IsPalettized(format) || IsPalettized(metadata.format))
        return HRESULT_E_NOT_SUPPORTED;
