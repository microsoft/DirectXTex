...
HRESULT GPUCompressBC::Prepare(size_t width, size_t height, uint32_t flags, DXGI_FORMAT format, float alphaWeight)
{
    ...
    case DXGI_FORMAT_BC7_UNORM:      m_srcformat = DXGI_FORMAT_R8G8B8A8_UNORM; break;

    case DXGI_FORMAT_BC7_UNORM_SRGB: m_srcformat = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB; break;

    default:                         m_bcformat = m_srcformat = DXGI_FORMAT_UNKNOWN; return HRESULT_E_NOT_SUPPORTED;
    }
...
HRESULT GPUCompressBC::Compress(const Image& srcImage, const Image& destImage)
{
    ...
    if (srcImage.width != destImage.width || srcImage.height != destImage.height || srcImage.width != m_width || srcImage.height != m_height
    ...
    case DXGI_FORMAT_BC7_UNORM_SRGB: isbc7 = true; break;
    ...
    while (num_blocks > 0)
    {
        ...
        if (isbc7)
        {
            ...
            RunComputeShader(pContext,
                m_BC7_tryMode456CS.Get(),
                pSRVs,
                2,
                m_constBuffer.Get(),
                m_err1UAV.Get(),
            ...
            if (m_bc7_mode137)
            {
                for (UINT i = 0; i < 3; ++i)
                {
                    ...
                    RunComputeShader(pContext,
                        m_BC7_tryMode137CS.Get(),
                        pSRVs,
                        2,
                        m_constBuffer.Get(),
                        (i & 1) ? m_err1UAV.Get() : m_err2UAV.Get(),
                        uThreadGroupCount);
                }
            }
            ...
            if (m_bc7_mode02)
            {
                ...
                for (UINT i = 0; i < 2; ++i)
                {
                    ...
                    RunComputeShader(pContext,
                        m_BC7_tryMode02CS.Get(),
                        pSRVs,
                        2,
                        m_constBuffer.Get(),
                        (i & 1) ? m_err2UAV.Get() : m_err1UAV.Get(),
                        uThreadGroupCount);
                }
            }
